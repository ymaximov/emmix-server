...
const voidSO = async (req, res) => {
    ...
    try {
        ...
        if (salesOrder && salesOrder.released) {
            // Check if there are any sales order items
            ...
            if (salesOrderItems.length === 0) {
                // If there are no sales order items, void the sales order
                ...
                return res.status(400).json({ message: 'Cannot void the sales order as it is already released and has sales order items.' });
            }
        }
